    ...
    <div className="flex gap-1" aria-label={`تقييم ${count} من 5`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <svg
          ...
          stroke="currentColor"
          strokeWidth="1.5"
          className={i < count ? "text-gold" : "text-gray-200"}
          aria-hidden="true"
        >
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
          {testimonials.map((t) => (
            <div
              key={t.name}
              className="bg-white border border-gray-100 shadow-sm rounded-2xl p-6 flex flex-col gap-4"
            >
              {/* النجوم */}
              <StarRating count={t.rating} />

              {/* النص */}
              <p className="text-dark/65 text-sm leading-relaxed flex-1">
                &ldquo;{t.text}&rdquo;
              </p>
              ...
              <div className="flex items-center gap-3 pt-2 border-t border-gray-100">
                ...
                <div>
                  ...
                  <p className="text-dark/45 text-xs">{t.area}</p>
                </div>
              </div>
            </div>
          ))}
